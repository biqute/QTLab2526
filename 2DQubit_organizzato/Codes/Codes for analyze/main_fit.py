...
if __name__ == '__main__':
    ...
    if a:   
        ...
        ax4.plot(Temperature, Q_internal_inverted, color='brown', marker='s', ls='-', ms=5)
        ...
        print(Q_internal_inverted[0], list_fresonance[0])
    
    fig = plt.figure(figsize=(15, 10))
    plt.plot(Temperature, Q_internal_inverted, color='brown', marker='s', ls='-', ms=5)
    plt.title('Perdite Interne ($1/Q_i$)')
    plt.xlabel('T [mK]')
    plt.ylabel('$1/Q_i$')
    plt.legend()
    plt.grid(True)
    plt.savefig("perdite_interne.png", dpi=300)
    plt.show()