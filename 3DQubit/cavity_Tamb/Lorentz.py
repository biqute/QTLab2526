...
# === Lettura dati ===
data_file = "cavity_7.29GHz" + "_corretta"
...
real = data[:, 1]
imag = data[:, 2]
phase = np.unwrap(np.atan(imag/real))
...
gamma_guess = (f.max() - f.min()) / 100  # stima grossolana della larghezza (es. 1% del range)
A_guess = y.max() - y.min()
y0_guess = y.min()  # stima del livello di fondo
...
#----Signal plot-----
ax_mag.plot(f/1e9, y, '.', label="Data")
ax_mag.plot(f_fit/1e9, y_fit, '-', label="Fit")
