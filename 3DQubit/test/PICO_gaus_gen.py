...
# 1. Creiamo l'asse X per la funzione matematica (da -3 a 3 copre bene la campana)
        x = np.linspace(-3, 3, WAVEFORM_SAMPLES)

# 2. Calcoliamo la funzione gaussiana: e^(-x^2)
# I valori andranno da ~0 (alle estremità) a 1.0 (al centro)
        gauss_normalized = np.exp(-np.square(x))

# 3. Trasformiamo i valori in int16 per il buffer del PicoScope
# Mappiamo lo 0.0 al valore minimo e l'1.0 al valore massimo dell'AWG
        span = max_val.value - min_val.value
        waveform = (gauss_normalized * span + min_val.value).astype(np.int16)
