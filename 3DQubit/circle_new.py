...
data = np.load("data_power_0dBm.npz") #Assumi che il file contenga: freq, real, imag
struct = data['0']

mask = (struct['freq'] > 7.413e9) & (struct['freq'] < 7.416e9)  # Applica una soglia per filtrare i dati
frequencies = struct['freq'][mask]/1e9  # Frequenze in GHz
signal = struct['signal'][mask]
phase = struct['phase'][mask]
